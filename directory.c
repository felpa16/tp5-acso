...
int directory_findname(struct unixfilesystem *fs, const char *name, int dirinumber, struct direntv6 *dirEnt) {
    ...
    if ((inode.i_mode & IFDIR) == 0) return -1;  // Check if it's a directory

    int size = inode_getsize(&inode);
    int block_count = (size + DISKIMG_SECTOR_SIZE - 1) / DISKIMG_SECTOR_SIZE; // Number of blocks

    struct direntv6 *dirents = (struct direntv6 *) malloc(DISKIMG_SECTOR_SIZE);
    if (dirents == NULL) return -1;
    ...
    if (size > 7 * 256 * DISKIMG_SECTOR_SIZE) {
        ...
        block_count = (size + 256 * DISKIMG_SECTOR_SIZE - 1) / (256 * DISKIMG_SECTOR_SIZE);
    }
